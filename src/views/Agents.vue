    ...
    methods: {
      async fetchAgents() {
        ...
        try {
          ...
          const res = await axios.get("http://localhost:4001/api/agents", {
      ...
      async createAgent() {
        try {
          const token = localStorage.getItem("token");
          await axios.post("http://localhost:4001/api/agents", this.form, {
            headers: { Authorization: `Bearer ${token}` },
          });
      ...
      async deleteAgent(id) {
        ...
        try {
          const token = localStorage.getItem("token");
          await axios.delete(`http://localhost:4001/api/agents/${id}`, {
            headers: { Authorization: `Bearer ${token}` },
          });
